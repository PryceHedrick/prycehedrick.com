...
function ClientCard({ project }: { project: (typeof projects)[number] }) {
  return (
    <div
      className={`bg-[#111827] rounded-3xl overflow-hidden flex flex-col ring-1 ${project.ringColor} transition-all duration-200 ${
        project.name === "Panda International Order Management System"
          ? "hover:shadow-[0_0_40px_rgba(217,119,6,0.12)]"
          : "hover:shadow-[0_0_40px_rgba(14,165,233,0.10)]"
      }`}
    >
      ...
      <div className="p-8 flex flex-col flex-grow">
        ...
        <h2 className="text-2xl font-bold mb-3">
          {project.liveUrl ? (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="hover:text-[#0ea5e9] transition-colors">
              {project.name} ↗
            </a>
          ) : (
            project.name
        ...
        <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl px-5 py-4 mb-5">
          <div className="text-xs font-semibold text-[#0ea5e9] uppercase tracking-wider mb-1">Result</div>
        ...
        <div className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <span key={t} className="px-2.5 py-1 rounded-full text-xs font-medium bg-[#1E293B] text-[#94A3B8]">
              {t}
            </span>
          ))}
        </div>
...
function PersonalCard({ project }: { project: (typeof projects)[number] }) {
  return (
    <div className="bg-[#111827] border border-[#1E293B] rounded-3xl overflow-hidden flex flex-col transition-all duration-200 hover:shadow-[0_0_30px_rgba(14,165,233,0.08)] hover:border-[#334155]">
      <div className="relative bg-[#0A0F1E] border-b border-[#1E293B]">
        <Image
          src={project.screenshot}
          alt={`${project.name} — screenshot`}
      ...
      <div className="p-8 flex flex-col flex-grow">
        ...
        <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl px-4 py-3.5 mb-5">
          <div className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wider mb-1">Status</div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                filter === f
                  ...
                  : "text-[#94A3B8] hover:text-white border border-[#1E293B]"
              }`}
            >
              {f === "all" ? "All Work" : f === "client" ? "Client Projects" : "Personal Builds"}
            </button>
          ))}
        </div>
      </div>
        ...
        <section className="px-6 lg:px-8 pb-8 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
            {clientProjects.map((project) => (
              <ClientCard key={project.name} project={project} />
            ))}
          </div>
        </section>
      )}
